      ...
      <div class="form-row">
        <div class="form-item">
          <div class="label-row">
            ...
            <el-button v-if="aiConfigured" link type="primary" size="small" @click="onSavePreset">
...
import { testAiConnection, testEmbeddingConnection } from '../../api/settings'
import { useSettingsForm } from '../../composables/useSettingsForm'
import {
  Cpu, Connection, CircleCheckFilled, CircleCloseFilled,
  Close, Clock, FolderAdd, Key
} from '@element-plus/icons-vue'

const {
  form, aiPresets, aiConfigured,
  doSave, loadAiPreset, handleSaveAiPreset, handleDeleteAiPreset
} = useSettingsForm()
...
// Clear test results on form change
watch(() => form.value.ai_api_key, () => { testResult.value = null; embeddingTestResult.value = null })
watch(() => form.value.ai_base_url, () => { testResult.value = null })
watch(() => form.value.ai_model, () => { testResult.value = null })
...
// Expose detectProvider for parent to call after loading settings
defineExpose({ detectProvider })
...
.provider-option { display: flex; justify-content: space-between; align-items: center; }
.provider-label { font-weight: 500; }
.provider-hint { font-size: 12px; color: #8b949e; }
.model-option { display: flex; justify-content: space-between; align-items: center; gap: 8px; }
...
.test-error { background: #fef2f2; border: 1px solid #fecaca; color: #991b1b; }

@media (max-width: 767px) {
  .form-row.two-col { flex-direction: column; gap: 16px; }
  .card-header { padding: 16px 18px; flex-wrap: wrap; }
  .card-header-actions { width: 100%; margin-top: 8px; }
